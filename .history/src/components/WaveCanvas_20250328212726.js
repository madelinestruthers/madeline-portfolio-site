...
export default function WaveCanvas() {
  ...
  const [canvasHeight, setCanvasHeight] = useState(280); // Increased height for more solid blue space

  useEffect(() => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext("2d");
    let width = window.innerWidth;
    let height = width < 768 ? 180 : 280;
    let animationId;
    let timeOffset = 0;
    let mouseX = width / 2;
    let mouseY = height / 2;
    ...
    const handleMouseMove = (e) => {
      ...
      if (distanceY < 150) {
        mouseX = e.clientX;
        mouseY = e.clientY;
        targetAmplitude = 40 + (150 - distanceY) * 0.2;
    ...
    const draw = (t) => {
      ...
      const parallaxOffset = scrollFactor * 30;

      drawWave(t, "210, 82%, 15%", animatedAmplitude, 0.0005, 1, glowStrength, parallaxOffset);
