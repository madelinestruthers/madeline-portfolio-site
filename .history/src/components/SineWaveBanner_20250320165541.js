...
const SineWaveBanner = () => {
    ...
    class WavesCanvas {
      constructor(elm, options = {}) {
        ...
        this.settings = {
          ...
          lineWidth: parseInt(data.lineWidth) || options.lineWidth || 1.8, // Slightly thicker for smoothness
        ...
        draw(ctx) {
          ...
          for (let x = 0; x <= this.canvas.width; x += 20) {
            ...
            const y =
              this.yOffset +
              offset +
              Math.sin(x * 0.005 + this.phase + mouseX * 0.0005) * this.settings.amplitude +
              Math.cos(x * 0.002 + this.phase + mouseX * 0.0003) * this.settings.amplitude * 0.5;
            ctx.lineTo(x, y);
          }
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "450px", overflow: "hidden", backgroundColor: "#1F5A80" }}>
